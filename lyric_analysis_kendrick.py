...
class Lyrics(object):
	def grab_links(self, main_page):
		...
		arr_post = [item.encode('utf-8').lower().replace(" ","-").replace("'", "").replace("/","") for item in arr_post]
	
		for i in arr_post:
			final_links.append("http://www.metrolyrics.com/" + i + "-kendrick-lamar.html")
		return final_links # How to deal with featured songs?
	...
	def corpus_creation(self, final_links):
		...
		for link in final_links:
			...
			print lyrics
			with open("kendrick_lyrics.txt", "a") as the_file:
	...
	def words_per_line(self):
		...
		"""
		total_words = []
		with open("kendrick_lyrics.txt", "r") as f:
			for line in f.readlines():
				total_words.append(len(line.split(' ')))
		return "Average words per line: ", np.mean(total_words)

sol = Lyrics()
main_page = requests.get("http://www.metrolyrics.com/kendrick-lamar-lyrics.html")
file_content = open("kendrick_lyrics.txt").read()
